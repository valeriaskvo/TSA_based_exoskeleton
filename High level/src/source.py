...
class Computer:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            a = np.asarray(result)
            np.savetxt(self.file_name, a, delimiter=";", header=";".join(headers))

            headers = list(self.motor_data[0].keys())
            data = np.asarray([list(f.values()) for f in self.motor_data])
            np.savetxt(f"motor_{self.file_name}", data, delimiter=";", header=";".join(headers))
