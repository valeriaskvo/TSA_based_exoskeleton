...
def force(msg: can.Message) -> None:
    global force_data, c_s
    board_id = msg.arbitration_id
    device_id, flag, *data = list(msg.data)
    data = msg.data[-2:]
    mass_raw = int.from_bytes(data, "little", signed=True)
    print(f"Device {board_id}: {device_id} with mass {mass_raw / 65535 * 3.6}")
    force_data.append(mass_raw / 65535 * 3.6)
    c_s += 1
