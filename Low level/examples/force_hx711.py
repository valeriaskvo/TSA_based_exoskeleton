...
class Scales(HX711):
    ...
    def filter_buffer(self, reads=10, delay_us=100):
        buffer = []
        for i in range(reads):
            buffer.append(self.read())
            sleep_us(delay_us)
        return sum(buffer) / reads
...
def filter_buffer(sens, reads=10, delay=0.0125):
    buffer = []
    for i in range(reads):
        val = sens.read()
        buffer.append(val)
...
def kalman_filter(sens: HX711, reads=10):
    prev = 0
...
if __name__ == '__main__':
    ...
    try:
        sensor.power_on()
        offset = filter_buffer(sensor)
        n = 0
        m = 0
        while True:
            z = sensor.read() - offset
            n += 1
            m = (n-1) / n * m + 1 / n * z
            print(m, z)
